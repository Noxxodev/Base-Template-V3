...
local function checkPatient(target)
    ...
    local data = lib.callback.await('ambulancejob:getData', false, targetServerId)
    ...
    if isDead then
        options[#options + 1] = {
            ...
            onSelect = function()
                ...
                TriggerServerEvent("ambulancejob:healPlayer", dataToSend)
            end,
        }
...
function createDistressCall()
    ...
    TriggerServerEvent("ambulancejob:createDistressCall", data)
...
function openDistressCalls()
    ...
    local distressCalls = lib.callback.await('ambulancejob:getDistressCalls', false)
                ...
                lib.registerContext({
                    ...
                    options = {
                        ...
                        },
                        {
                            ...
                            onSelect    = function()
                                TriggerServerEvent("ambulancejob:callCompleted", call)
...
RegisterNetEvent("ambulancejob:openDistressCalls", openDistressCalls)
...
RegisterNetEvent("ambulancejob:playHealAnim", function(data)
    local playerPed = cache.ped or PlayerPedId()
...
RegisterNetEvent("ambulancejob:createDistressCall", function(name)
    if not Framework.hasJob(emsJobs) then return end
