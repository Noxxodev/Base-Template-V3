    ...
    if source ~= 0 then
        if isAllowedToChange(source) then
            ...
            if not DynamicWeather then
                TriggerClientEvent('vSync:notify', source, 'Les changements météo dynamiques sont maintenant ~r~désactivés~s~.')
            else
                TriggerClientEvent('vSync:notify', source, 'Les changements météo dynamiques sont maintenant ~b~activés~s~.')
            end
        else
            TriggerClientEvent('chatMessage', source, '', {255,255,255}, '^8Erreur : ^1Vous n’avez pas la permission d’utiliser cette commande.')
        end
...
RegisterCommand('weather', function(source, args)
    ...
    if args[1] == nil then
        ...
        if source == 0 then print(errorMsg) else TriggerClientEvent('chatMessage', source, '', {255,255,255}, errorMsg) end
        return
    end
...
RegisterCommand('blackout', function(source)
    ...
    elseif isAllowedToChange(source) then
        TriggerClientEvent('vSync:notify', source, blackout and 'Coupure de courant ~b~activée~s~.' or 'Coupure de courant ~r~désactivée~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('morning', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(9)
        TriggerClientEvent('vSync:notify', source, 'L’heure a été réglée sur le ~y~matin~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('noon', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(12)
        TriggerClientEvent('vSync:notify', source, 'L’heure a été réglée sur le ~y~midi~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('evening', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(18)
        TriggerClientEvent('vSync:notify', source, 'L’heure a été réglée sur le ~y~soir~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('night', function(source)
    ...
    if isAllowedToChange(source) then
        ShiftToMinute(0)
        ShiftToHour(23)
        TriggerClientEvent('vSync:notify', source, 'L’heure a été réglée sur la ~y~nuit~s~.')
        TriggerEvent('vSync:requestSync')
    end
end)
...
RegisterCommand('time', function(source, args)
    ...
    elseif isAllowedToChange(source) then
        ...
        TriggerClientEvent('vSync:notify', source, "L’heure a été modifiée.")
        TriggerEvent('vSync:requestSync')
    end
end)
