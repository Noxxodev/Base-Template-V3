...
AddEventHandler("esx:setJob2", function(_, job, lastJob)
    local lastKey = "job2_" .. lastJob.name .. ":count"
    local newKey = "job2_" .. job.name .. ":count"
    local currentLast = Core.JobsPlayerCount["job2_" .. lastJob.name]

    Core.JobsPlayerCount["job2_" .. lastJob.name] = ((currentLast and currentLast > 0) and currentLast or 1) - 1
    Core.JobsPlayerCount["job2_" .. job.name] = (Core.JobsPlayerCount["job2_" .. job.name] or 0) + 1

    GlobalState[lastKey] = Core.JobsPlayerCount["job2_" .. lastJob.name]
    GlobalState[newKey] = Core.JobsPlayerCount["job2_" .. job.name]
end)
