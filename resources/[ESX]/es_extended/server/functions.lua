...
local function checkTableJob2(key, val, xPlayer, xPlayers)
    for valIndex = 1, #val do
        local value = val[valIndex]
        if not xPlayers[value] then
            xPlayers[value] = {}
...
function ESX.GetExtendedPlayersJob2(key, val)
    if not key then
        return ESX.Table.ToArray(ESX.Players)
    end
    ...
    if type(val) == "table" then
        for _, xPlayer in pairs(ESX.Players) do
            checkTableJob2(key, val, xPlayer, xPlayers)
        end
...
function ESX.RefreshJobs()
    ...
    if not Jobs2 then
        -- Fallback data, if no jobs exist
        ESX.Jobs2["unemployed"] = { label = "Unemployed", grades = { ["0"] = { grade = 0, label = "Unemployed", salary = 200, skin_male = {}, skin_female = {} } } }
    else
        ESX.Jobs2 = Jobs2
    end
end
